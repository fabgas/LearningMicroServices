function updateMultiplication() {
    $.ajax({
        url: "http:///192.168.99.100:8000/api/multiplications/random"
    }).then(function (data) {
        // Cleans the form
...
function updateResults(alias) {
    ...
    $.ajax({
        async: false,
        url: "http://192.168.99.100:8000/api/results?alias=" + alias,
...
$(document).ready(function () {
    ...
    $("#attempt-form").submit(function (event) {
        ...
        $.ajax({
            url: 'http://192.168.99.100:8000/api/results',
            type: 'POST',
            ...
            success: function (result) {
                if (result.correct) {
                    $('.result-message').empty()
                        .append("<p class='bg-successtext - center'>The result is correct! Congratulations!</p > ");
                    } else {
                    $('.result-message').empty()
                        .append("<p class='bg-dangertext - center'>Oops that's not correct!But keep trying!</p > ");
}
            }
        });
        updateMultiplication();
